...
export const SchoolDashboardPage: React.FC<SchoolDashboardPageProps> = ({ user, role, onLogout, onBackToRoles }) => {
    ...
    if (viewingStudentPortalFor) {
        return (
            <StudentDashboardPage
                ...
                onLogout={onLogout}
                onBackToRoles={() => setViewingStudentPortalFor(null)}
    ...
    const renderContent = () => {
        if (role === RoleEnum.School) {
            ...
            switch (currentView) {
                case 'dashboard': return <SchoolDashboardOverviewTab school={school!} />;
                case 'profile': return <SchoolProfileTab schoolProfile={school!} onProfileUpdate={setSchool} />;
                ...
                case 'calendar': return <EventCalendarPage />;
                case 'homework': return <HomeworkManagementPage onBackToDashboard={() => setCurrentView('dashboard')} />;
                case 'settings': return <div className="p-8 text-center"><h2 className="text-2xl font-bold">{currentView.charAt(0).toUpperCase() + currentView.slice(1)}</h2><p className="mt-2 text-slate-500">This feature is coming soon!</p></div>;
                default: return null;
            }
        }
        ...
        if (role === RoleEnum.Parent) {
            switch (currentView) {
                case 'dashboard': return <ParentDashboardOverviewTab user={user} onNavigate={setCurrentView} />;
                case 'myChildren': return <ParentProfilePage user={user} onLogout={onLogout} onBackToRoles={onBackToRoles} onBackToDashboard={() => setCurrentView('dashboard')} onViewStudentPortal={setViewingStudentPortalFor} />;
                case 'shareCode': return <ParentShareCodePage user={user} onBackToDashboard={() => setCurrentView('dashboard')} />;
                case 'attendance': return <ParentAttendancePage onBackToDashboard={() => setCurrentView('dashboard')} />;
                case 'grades': return <ParentGradesPage onBackToDashboard={() => setCurrentView('dashboard')} />;
                case 'fees': return <ParentFinancialsPage onBackToDashboard={() => setCurrentView('dashboard')} />;
                case 'settings': return <div className="p-8 text-center"><h2 className="text-2xl font-bold">Settings</h2><p className="mt-2 text-slate-500">This feature is coming soon!</p></div>;
                default: return null;
            }
        }
        ...
        if (error && !school) {
            return (
                 <div className="flex flex-col items-center justify-center min-h-screen text-center p-4">
                    <p className="text-red-500 font-semibold">An error occurred:</p><p className="text-red-500 mt-2">{error}</p><button onClick={fetchSchoolProfile} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg">Try Again</button>
                </div>
            );
        }
    }
            ...
            <aside className="w-64 flex-shrink-0 bg-white dark:bg-slate-800 p-4 flex flex-col justify-between border-r border-slate-200 dark:border-slate-700">
                <div>
                    <div className="flex items-center gap-3 px-3 py-2 mb-6">
                        ...
                        <span className="text-xl font-bold text-slate-800 dark:text-white">{role === RoleEnum.School ? 'School Portal' : 'Parent Portal'}</span>
                    </div>
                ...
                <div>
                    ...
                    <div className="mt-6 p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm font-bold text-slate-800 dark:text-white truncate">{user.user_metadata?.fullName || 'User'}</p>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div ref={menuRef} className="relative">
                            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="flex items-center gap-2 p-1 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                             ...
                             {isMenuOpen && (
                                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-lg shadow-xl z-20 border dark:border-slate-700 py-1">
                                    <div className="p-3 border-b dark:border-slate-700">
                                        <p className="font-semibold text-sm text-slate-800 dark:text-slate-200">{user.user_metadata?.fullName}</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">{user.email}</p>
                                    </div>
                                    <a href="#" onClick={(e) => { e.preventDefault(); onBackToRoles(); }} className="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700">Change Role</a>
                                    <a href="#" onClick={(e) => { e.preventDefault(); onLogout(); }} className="flex items-center gap-3 px-4 py-2 text-sm text-red-500 hover:bg-red-50 dark:hover:bg-slate-700/50">
