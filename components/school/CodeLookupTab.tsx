...
export const CodeLookupTab: React.FC<CodeLookupTabProps> = ({ school }) => {
    ...
    const handleLookup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (codeData.type !== 'admission') {
                throw new Error(`This is an '${codeData.type}' code. An 'admission' code is required to admit a student.`);
            }
            ...
            // Log success
            await supabase.from('code_lookup_log').insert({
            ...
            // Log failure
             await supabase.from('code_lookup_log').insert({
                school_user_id: school.user_id,
                code: upperCaseCode,
                result_status: err.message.includes('not found') ? 'Invalid' : 'Expired',
            });
        } finally {
            setIsLoading(false);
        }
    ...
    const handleAdmitStudent = async () => {
        ...
        try {
            ...
            const { error: insertError } = await supabase.from('school_students').insert({
                school_id: school.id,
                student_id: result.code.child_id,
